...
class LocalQwen3Embedding:
    def __init__(self, server_url: str):
        ...
        try:
            # 使用 /v1/models 作为健康检查（返回 JSON，确认模型加载）
    ...
    def encode(self, texts: List[str]) -> List[np.ndarray]:
        if not self._available:
            return [np.random.rand(2560).astype(np.float32) for _ in texts]  # 维度匹配 Qwen 模型
        try:
            resp = requests.post(
            ...
            print(f"DEBUG: Status: {resp.status_code}, Body preview: {resp.text[:200]}")  # 调试
