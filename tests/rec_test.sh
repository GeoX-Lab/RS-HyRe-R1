...
for STEP in "${STEPS[@]}"; do
    ...
    # 临时修改脚本中的关键变量
    TEMP_SCRIPT="rec_test_step_${STEP}.py"
    cp "$TEST_SCRIPT" "$TEMP_SCRIPT"
    
    # 动态替换模型路径和输出路径
    sed -i "s|MODEL_PATH = .*|MODEL_PATH = \"${MODEL_PATH}\"|" "$TEMP_SCRIPT"
    sed -i "s|OUTPUT_PATH = .*|OUTPUT_PATH = \"${OUTPUT_DIR}/res_result_${STEP}.json\"|" "$TEMP_SCRIPT"
    ...
    echo "步长 ${STEP} 测试完成! 结果已保存至 ${OUTPUT_DIR}/res_result_${STEP}.json"
    echo
done
